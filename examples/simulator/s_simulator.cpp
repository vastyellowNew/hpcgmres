...
void simulation(struct s_single_shooting_cgmres *cgmres, 
  ...
  float s_current_state_vec[d_nmpc_model_dimx()], s_next_state_vec[d_nmpc_model_dimx()],
      s_control_input_vec[d_nmpc_model_dimu()];
  ...
  for (int i=0; i<d_nmpc_model_dimx(); i++) {
    ...
    s_current_state_vec[i] = initial_state_vec[i];
  }
  s_single_shooting_cgmres_initialize_solution(cgmres, start_time,   
                                               s_current_state_vec);
  s_single_shooting_cgmres_get_control_input(cgmres, s_control_input_vec);
  for (int i=0; i<d_nmpc_model_dimu(); i++) {
    control_input_vec[i] = s_control_input_vec[i];
  }
  ...
  for (double current_time=start_time; current_time<end_time; 
    ...
    saveData(d_nmpc_model_dimx(), d_nmpc_model_dimu(), state_data, 
             ...
             control_input_vec, 
             s_single_shooting_cgmres_get_error_norm(cgmres, current_time, s_current_state_vec));
    ...
    integrator.rungeKuttaGill(current_time, current_state_vec, 
                              ...
                              next_state_vec);
    for (int i=0; i<d_nmpc_model_dimx(); i++) {
      s_next_state_vec[i] = next_state_vec[i];
    }
    ...
    for (int i=0; i<d_nmpc_model_dimx(); i++) {
      s_current_state_vec[i] = current_state_vec[i];
    }
  }
